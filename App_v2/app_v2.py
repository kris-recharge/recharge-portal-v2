...
def _load_evse_pricing_rows() -> pd.DataFrame:
    """Load EVSE pricing rules from Postgres (Supabase)."""
    try:
        # Reuse the same DB config as the other loaders.
        # load_meter_values() etc. call into rca_v2.loaders, which reads env vars.
        from rca_v2.loaders import get_engine  # local import to avoid circulars
...
def _attach_estimated_revenue(sess: pd.DataFrame) -> pd.DataFrame:
    """Attach an 'Estimated Revenue' column to the sessions DataFrame."""
    if sess is None or sess.empty:
        return sess

    pricing = _load_evse_pricing_rows()
    out = sess.copy()

    if pricing is None or pricing.empty or "station_id" not in out.columns:
        out["Estimated Revenue"] = None
        return out

    # Normalize timestamps
    if "start_time_utc" in out.columns:
        starts = pd.to_datetime(out["start_time_utc"], utc=True, errors="coerce")
    else:
        starts = pd.to_datetime(out.get("Start Date/Time"), utc=True, errors="coerce")
    out["__start_utc__"] = starts

    pricing = pricing.copy()
    pricing["effective_start"] = pd.to_datetime(pricing["effective_start"], utc=True, errors="coerce")
    pricing["effective_end"] = pd.to_datetime(pricing["effective_end"], utc=True, errors="coerce")
    pricing = pricing.sort_values(["station_id", "effective_start"], ascending=[True, True])

    def pick_rule(station_id: str, start_utc: pd.Timestamp):
        if not station_id or pd.isna(start_utc):
            return None
        rules = pricing[pricing["station_id"] == station_id]
        if rules.empty:
            return None
        rules = rules[rules["effective_start"].notna() & (rules["effective_start"] <= start_utc)]
        if rules.empty:
            return None
        rules = rules[(rules["effective_end"].isna()) | (rules["effective_end"] > start_utc)]
        if rules.empty:
            return None
        return rules.iloc[-1]

    est_vals: list[float | None] = []
    for _, r in out.iterrows():
        rule = pick_rule(str(r.get("station_id", "")), r.get("__start_utc__"))
        if rule is None:
            est_vals.append(None)
            continue
        conn_fee = float(rule.get("connection_fee") or 0)
        p_kwh = float(rule.get("price_per_kwh") or 0)
        p_min = float(rule.get("price_per_min") or 0)

        try:
            energy = float(r.get("Energy Delivered (kWh)") or 0)
        except Exception:
            energy = 0.0
        try:
            dur = float(r.get("Duration (min)") or 0)
        except Exception:
            dur = 0.0

        est = conn_fee + (energy * p_kwh) + (dur * p_min)
        est_vals.append(round(est, 2))

    out["Estimated Revenue"] = est_vals
    out.drop(columns=["__start_utc__"], inplace=True, errors="ignore")
    return out
...
def load_mv_auth_and_sessions(stations_key, start_iso, end_iso):
    ...
    sess = _attach_estimated_revenue(sess)
